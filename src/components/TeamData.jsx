...
export const TeamData = () => {
    ...
    const {username, role, designation, teamlead, id} = userData;
    ...
    const getTeamInfo = async () => {
        if(designation === "ATL" || designation === "TL" || designation === "DM") {
            await axios.get(`http://localhost:3000/user/${username}`)
            .then((res)=>{
                setRecords(res.data.team);
                console.log(res.data.team.length);
            })
            .catch((err)=>{
                console.log(err);
            })
        }else {
            await axios.get(`http://localhost:3000/user/${teamlead}`)
            .then((res)=>{
                console.log(res.data.team);
                res.data.team.map(each => {
                    if(each.id === id) {
                        setRecords(each.team);
                    }
                })
            })
            .catch((err)=>{
    ...
    const submitForm = async(e) => {
        ...
        if(designation === "ATL" || designation === "TL" || designation === "DM") {
            ...
            .then((res)=>{
                console.log(res.data.team);
                let updatedData = res.data.team.map(each => {
                    if(each.id === id) {
                        // setRecords(each.team);
                        each.team.push(newUserData);
                    }
                    return each
                })
    ...
    const adminView = () => {
        return (
            ...
            {records.length > 0 ? (
                ...
                <div><h1>No Team Members Found</h1></div>
            )}
            </div>
        )
    }
  ...
  return (
    <div>
        ...
        {designation === "TM" ? userView() : adminView()}
